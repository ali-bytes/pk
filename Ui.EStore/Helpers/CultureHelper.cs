...
namespace Ui.EStore.Helpers
{
    ...
    {
        readonly static UserSession UserSession = new UserSession();
        ...
        public static Enums.Lang GetLang()
        {
            string lang = GetNeutralCulture(Thread.CurrentThread.CurrentCulture.Name);
            string[] tmp = lang.Split('_');
            if (tmp[0] == "ar")
            {
                UserSession.UserLanguage = new LanguageService(new UnitOfWork()).GetLanguageByLangType("Ar");
                return Enums.Lang.Ar;
            }
            UserSession.UserLanguage = new LanguageService(new UnitOfWork()).GetLanguageByLangType("En");
            return Enums.Lang.En;
        }

        public static string GetUserLanguageValue(string toolName, string pageName)
        {
            if (string.IsNullOrEmpty(toolName.Trim()))
            {
                toolName = "NotSetValue" + pageName;
            }


            var languageService = new LanguageService(new UnitOfWork());
            return languageService.GetUserLanguageValue(pageName, toolName);
        }

        public static string GetUserLanguageValue(string toolName)
        {
            if (string.IsNullOrEmpty(toolName))
            {
                toolName = "NotSetValue" + HttpContext.Current.Request.AppRelativeCurrentExecutionFilePath;
            ...
            var languageService = new LanguageService(new UnitOfWork());
            return languageService.GetUserLanguageValue(toolName, HttpContext.Current.Request.AppRelativeCurrentExecutionFilePath);
        }

        public static SeoLanguageTool GetSeoUserLanguageValue(string toolName, string pageName = null, string pageUrl = null)
        {
            if (pageName == null)
            {
                pageName = HttpContext.Current.Request.AppRelativeCurrentExecutionFilePath;
            }
            if (string.IsNullOrEmpty(toolName.Trim()))
            {
                toolName = pageName;
            ...
            var languageService = new LanguageService(new UnitOfWork());
            return languageService.GetUserSeoLanguageValue(toolName, pageName, pageUrl);
        }


        public static string GetSeoValue(string toolName, string pageName = null, string pageUrl = null)
        {
            if (pageName == null)
            {
                pageName = HttpContext.Current.Request.AppRelativeCurrentExecutionFilePath;
            ...
            var languageService = new LanguageService(new UnitOfWork());
            return languageService.GetUserSeoLanguageValue(toolName, pageName, pageUrl).SeoUrl;
        }
    }
}